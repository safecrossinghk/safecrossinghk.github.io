...
// ================================
// 香港分区即时天气 —— 坐标对照表
// 注意：这里的 pos 沿用原本战场场景的 3D 坐标系（战场比例，非真实经纬度）
// 之后可依需要微调这几个位置，让它们对齐地形
// ================================
...
async function fetchWeatherData() {
  try {
    ...
    if (!response.ok) {
      throw new Error(`HKO API 回应异常: ${response.status}`);
    }
    const data = await response.json();
    const temperatureData = data.temperature?.data ?? [];

    console.group("🌡️ HKO 温度资料除错");
    console.log("API 回传时间戳记：", data.temperature?.recordTime ?? "未知");
    console.log("总共收到站数：", temperatureData.length);
    console.table(temperatureData);

    const targetStations = Object.keys(stationCoordinates);
    targetStations.forEach((name) => {
      const found = temperatureData.find((s) => s.place === name);
      if (found) {
        console.log(`✅ 找到「${name}」：${found.value}°${found.unit}`);
      } else {
        console.warn(`⚠️ 找不到「${name}」，请检查 API 回传的站名是否有差异（例如简繁、空格）`);
      }
    });
    console.groupEnd();
...
function updateWeatherLabels(temperatureData) {
  temperatureData.forEach((station) => {
    ...
    const label = makeLabel(text, coord.color, 3.6);
